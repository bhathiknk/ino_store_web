...
beforeEach(() => {
    ...
    // B. Setting a mock token in localStorage if your component uses it
    localStorage.setItem('token', 'test-token');

    // C. Mocking Axios GET requests
    ...
    // C. Mocking Axios PUT requests if your component makes any
...
test('renders products and handles image navigation', async () => {
    ...
    const nextButton = screen.getAllByRole('button', { name: '>' })[0]; // Adjust selector if necessary
    ...
    const prevButton = screen.getAllByRole('button', { name: '<' })[0]; // Adjust selector if necessary
...
test('displays correct product information', async () => {
    ...
    // A. Updated currency to USD to match the component
