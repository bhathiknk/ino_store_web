...
export default function AddressContent() {
  ...
  const handleCheckoutClick = () => {
    navigate('/checkout', { state: { address: addressDetails } }); // Pass address details to checkout page
  };
    ...
    const fetchAddressDetails = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setAddressDetails({
            id: address._id, // Save the address ID
  ...
  const handleSaveClick = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
          `http://localhost:5000/api/address/update/${addressDetails.id}`, // Use the saved address ID
          {
      ...
      if (response.status === 200) {
        ...
        setAddressDetails({
          id: response.data._id, // Ensure the ID is up-to-date
          address: response.data.address,
          province: response.data.province,
          zip: response.data.zipcode,
          mobile: response.data.contactNumber,
        }); // Update with the new address details
        toast.success('Address updated successfully!'); // Show success toast
      }
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to update address');
      toast.error(err.response?.data?.message || 'Failed to update address'); // Show error toast
    }
  };
